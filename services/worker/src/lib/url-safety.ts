...
async function assertPublicDns(hostname: string): Promise<void> {
  ...
  if (net.isIP(hostname)) {
    if (isBlockedIpAddress(hostname)) throw new Error("Webhook URL must not target private networks");
    return;
  }

  const records = await dns.lookup(hostname, { all: true, verbatim: true });
  if (!records.length) throw new Error("Webhook host did not resolve");
  const blocked = records.find((record) => isBlockedIpAddress(record.address));
  if (blocked) throw new Error("Webhook host resolves to a private network");
}

export async function validateOutboundWebhookUrl(rawUrl: string): Promise<string> {
  ...
  if (parsed.protocol !== "https:" && !(allowHttp && parsed.protocol === "http:")) {
    throw new Error(env.NODE_ENV === "production" ? "Webhook URL must use HTTPS" : "Webhook URL must use HTTP or HTTPS");
  }
